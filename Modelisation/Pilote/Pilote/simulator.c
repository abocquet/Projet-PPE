...
void acquerir_acceleration(double* acceleration){
	...
	nouvelle_vitesse[1] = vitesse[1] + (-k / masse * pow(vitesse[1],2)  +  prop_x / masse) * delta_t ;
	nouvelle_vitesse[2] = vitesse[2] + (- gravite * (masse - helium) + prop_y) / masse * delta_t ;
	...
	if(fabs(vitesse[1]) < 0.01 && sqrt(fabs(prop_x/k)) < 0.01){
...
void add_log(double* acceleration, double* vitesse, double* position, double* target, int prop_y, int prop_x, double angle_gauche, double angle_droit, double angle_nord){
	current_time += delta_t ;
	
	fprintf(log_file, "%f;%f;%f;%f;%f;%f;%f;%f;%f;%f;%f;%f;%f;%d;%d;%f;%f;%f;\n", current_time, acceleration[0], acceleration[1], acceleration[2], vitesse[0], vitesse[1], vitesse[2], position[0], position[1], position[2], target[0] - position[0], target[1] - position[1], target[2] - position[2], prop_y, prop_x, angle_gauche, angle_droit, angle_nord);
